...
int HashTable::computeHash(const string &s) {
    ...
    int totalAscii = 0;
    //for each character in s, add its ASCII value to stringHash (totalAscii)
    for (unsigned int i = 0; i < s.size(); ++i) {
        totalAscii += static_cast<int>(s.at(i));
    }
    //return remainder
    return totalAscii % size;
...
void HashTable::put(const string &s, int score) {
    unsigned int index = computeHash(s); //get index of the string in the array
    list<WordEntry>::iterator i; //helps iterate through list 
    //if the string is already in array (need to just add another appearance in list)
    if (contains(s)) {
        //at the index of the string, iterate through the linked list until getting to the word
        for (i = hashTable[index].begin(); i != hashTable[index].end(); ++i) {
            //found the word
            if (s == i->getWord()) {
                i->addNewAppearance(score); //inc numAppearances + score for the word since it's already there
            }
        }
    }
    //string is not in the array
    else {
        WordEntry newEntry = WordEntry(s, score); //create new WordEntry object to put into hashTable
        hashTable[index].push_back(newEntry); //push_back into array (add onto the end)
    }
...
double HashTable::getAverage(const string &s) {
    ...
    for (i = hashTable[index].begin(); i != hashTable[index].end(); ++i) {
        //found the word
        if (s == i->getWord()) {
            return i->getAverage(); //return the object's average
        }
    }
    return 2.0; //default
}
